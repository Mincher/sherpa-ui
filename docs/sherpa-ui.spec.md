...
**Independently testable by:** New component passes the §11 audit ACs and is surfaced by the MCP server.
...
| REQ-21 | Component file headers **SHALL** follow the JSDoc tag conventions defined in [`docs/COMPONENT-API-STANDARD.md`](COMPONENT-API-STANDARD.md) so the MCP schema parser can read them. |
...
| REQ-25 | `patterns/index.json` **SHALL** be regenerated by `npm run build` from source without manual edits. Component schemas are derived from JSDoc at query time by the MCP server. |
...
| REQ-105 | `patterns/index.json` **SHALL** be reproducible from source; manual edits to it are forbidden. |
...
*   **Generated artifacts:** `patterns/index.json`. Component schemas are parsed on demand by the MCP server.
...
`sherpa-nav`, `sherpa-nav-item`, `sherpa-breadcrumbs`, `sherpa-pagination`, `sherpa-tabs`, `sherpa-nav-section`.
