...
const DAU = Array.from({ length: 14 }, (_, i) => {
  ...
  return { date: fmt(new Temporal.PlainDate(2026, 1, i + 1)), value };
});

const RESPONSE_TIME = [];
for (let i = 0; i < 12; i++) {
  const date = fmt(new Temporal.PlainDate(2026, 2, i + 1));
...
const STORAGE = Array.from({ length: 12 }, (_, i) => ({
  date:  new Temporal.PlainDate(2026, i + 1, 1).toLocaleString('en-US', { month: 'short' }),
