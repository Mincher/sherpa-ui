...
export class SherpaNavPromo extends SherpaElement {
  ...
  static get observedAttributes() {
    return [...super.observedAttributes, 'data-promo-title', 'data-promo-message', 'data-promo-link-text', 'data-promo-link-url'];
  }
  ...
  #applyConfig() {
    ...
    const msgEl = this.$('.promo-message');

    if (titleEl) titleEl.textContent = title || '';
    if (msgEl) {
      if (link?.text) {
        msgEl.innerHTML = `${message || ''} <span class="promo-link">${link.text}</span>`;
        const linkEl = msgEl.querySelector('.promo-link');
        if (linkEl && link.url) linkEl.dataset.url = link.url;
      } else {
