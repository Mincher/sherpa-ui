...
export class SherpaMetric extends SherpaTable {
  ...
  isVisible() { return this.dataset.visible !== 'false'; }
  ...
  onAttributeChanged(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data-label':
        if (this.$('.metric-heading')) this.$('.metric-heading').textContent = newValue || '';
        break;
      case 'value':
        if (this.$('.metric-value')) this.$('.metric-value').textContent = newValue || '';
        break;
      case 'data-delta':
        if (this.$('.metric-delta')) this.$('.metric-delta').textContent = newValue || '';
        break;
      case 'data-status': {
  ...
  getTransferableConfig(presentationType) {
    return this.#contentData ? getTransferableConfig(this.#contentData, presentationType) : null;
  }
  ...
  #initialize() {
    ...
    const viewOptions = this.getViewOptions({ activeType: "kpi-metric", canShowChart: true });
  ...
  #populate() {
    ...
    if (explicitStatus && explicitStatus !== 'default' && explicitStatus !== 'none') {
    ...
    this.#updateSparkline({ status: cardStatus, unitText, values: sparklineValues, forceHidden: this.#suppressSparkline });
  ...
  #updateSparkline({ status, unitText, values, forceHidden } = {}) {
    ...
    const effectiveStatus = status !== undefined ? status : (this.$('.metric-card')?.dataset.status || '');
