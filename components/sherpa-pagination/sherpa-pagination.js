...
export class SherpaPagination extends SherpaElement {
  ...
  set pageSize(v) {
    this.setAttribute('data-page-size', String(Math.max(1, parseInt(v, 10) || 25)));
  }
  ...
  set totalRows(v) {
    this.setAttribute('data-total-rows', String(Math.max(0, parseInt(v, 10) || 0)));
  }
  ...
  #onHostClick(e) {
    ...
    else if (btn.classList.contains('page-prev')) newPage = Math.max(1, page - 1);
    else if (btn.classList.contains('page-next')) newPage = Math.min(totalPages, page + 1);
  ...
  goToPage(n) {
    const clamped = Math.max(1, Math.min(parseInt(n, 10) || 1, this.totalPages));
  ...
  setTotalRows(count) {
    this.setAttribute('data-total-rows', String(Math.max(0, parseInt(count, 10) || 0)));
