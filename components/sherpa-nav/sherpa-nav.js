...
export class SherpaNav extends SherpaElement {
  ...
  static get cssUrl() { return new URL('./sherpa-nav.css', import.meta.url).href; }
  static get htmlUrl() { return null; }

  static get observedAttributes() { return [...super.observedAttributes, 'data-src', 'data-home-href']; }
  ...
  isFavorite(itemId) {
    return !!this.$(`.nav-section[data-section-id="favorites"] sherpa-nav-item[data-item-id="${itemId}"]`);
  }

  setFavorite(itemId, label, route, on) {
    const sec = this.$('.nav-section[data-section-id="favorites"]');
    if (!sec) return;
    const existing = sec.querySelector(`sherpa-nav-item[data-item-id="${itemId}"]`);
    if (on && !existing) {
      sec.appendChild(this.#createNavItem({ id: itemId, label, route }, sec.dataset.editable === 'true'));
    } else if (!on && existing) {
      existing.remove();
    }
  ...
  addToRecent(itemId, label, route) {
    ...
    const newItem = this.#createNavItem({ id: itemId, label, route }, sec.dataset.editable === 'true');
  ...
  #wireToggleListeners() {
    ...
    sections.addEventListener('toggle', (e) => {
      ...
      if (details.open && details.matches('.nav-section')) {
        this.#emit('navsectionexpand', { sectionId: details.dataset.sectionId });
      }
    }, true);
  }
  ...
  #setInitialActiveState() {
    ...
    const target = (p === this.homeHref || p.endsWith('/home.html') || p === '/' || p.endsWith('/index.html'))
      ? 'home' : p.endsWith('/settings.html') ? 'settings' : null;
  ...
  #syncInitialState() {
    ...
    if (this.isPinned) this.closest('.sherpa-app-layout')?.setAttribute('data-nav-pinned', 'true');
  }
  ...
  #onHostClick(e) {
    const navItem = e.composedPath().find(n => n instanceof HTMLElement && n.tagName === 'SHERPA-NAV-ITEM');
    if (!navItem) return;

    // Delete button (shadow DOM → composedPath)
    if (e.composedPath().some(n => n instanceof HTMLElement && n.matches?.('.nav-item-delete'))) {
      e.preventDefault();
      e.stopPropagation();
      const sec = navItem.closest('.nav-section');
      if (navItem.dataset.itemId) {
        this.#emit('navitemdelete', { itemId: navItem.dataset.itemId, sectionId: sec?.dataset.sectionId || null });
      }
      navItem.remove();
      return;
    }
    ...
    // Drag handle — ignore
    if (e.composedPath().some(n => n instanceof HTMLElement && n.matches?.('.nav-item-drag'))) return;

    // Static targets: home, settings, search
    if (navItem.dataset.navTarget) {
      if (navItem.dataset.navTarget === 'search') { this.startSearch(); return; }
      this.#clearAllActiveStates();
  ...
  #createNavItem(item, editable = false) {
    const el = document.createElement('sherpa-nav-item');
    el.dataset.variant = 'child';
  ...
  #setupDragDrop() {
    this.$$('.nav-group[data-draggable="true"]').forEach(container => {
      ...
      setupDragSort(container, {
        itemSelector: ':scope > .nav-section', handleSelector: '.nav-item-drag',
        idAttribute: 'sectionId', isEnabled: () => this.isEditing,
        onReorder: (order) => {
          this.#emit('navsectionreorder', { groupIndex: gi, sectionOrder: order });
        }
      });
    });
  }
  ...
  #revealAncestors(node) {
    ...
    while (el) {
      ...
      else if (el.matches('.nav-subsection, .nav-section')) { delete el.dataset.searchHidden; el.open = true; }
      else if (el.matches('.nav-group')) delete el.dataset.searchHidden;
      el = el.parentElement?.closest('sherpa-nav-item, .nav-subsection, .nav-section, .nav-group') || null;
    }
  }
  ...
  #applySearchFilter(value) {
    ...
    const items = [...scope.querySelectorAll('sherpa-nav-item[data-variant="child"]')];
    const headers = [...scope.querySelectorAll('sherpa-nav-item:is([data-variant="section"], [data-variant="subsection"])')];
    const details = [...scope.querySelectorAll('.nav-section, .nav-subsection')];
    ...
    details.forEach(d => { d.dataset.searchWasOpen ??= d.open ? 'true' : 'false'; delete d.dataset.searchHidden; });
    groups.forEach(g => delete g.dataset.searchHidden);
    CSS.highlights.delete('nav-search-match');

    if (!filter) {
      details.forEach(d => { d.open = d.dataset.searchWasOpen === 'true'; delete d.dataset.searchWasOpen; });
      scope.querySelectorAll('[data-search-hidden]').forEach(el => delete el.dataset.searchHidden);
      return;
    }
    ...
    details.forEach(d => { d.dataset.searchHidden = 'true'; d.open = true; });
    ...
    if (ranges.length) CSS.highlights.set('nav-search-match', new Highlight(...ranges));
  }
}
