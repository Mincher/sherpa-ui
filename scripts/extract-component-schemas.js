...
function parseJSDoc(lines) {
  ...
  while (i < lines.length && !lines[i].startsWith("@")) {
    ...
    // Skip bare "filename.js" or "filename.ts" lines
...
function discoverComponents() {
  ...
  for (const entry of dirs) {
    ...
    const tsFile = path.join(COMPONENTS_DIR, entry.name, `${entry.name}.ts`);
    const jsFile = path.join(COMPONENTS_DIR, entry.name, `${entry.name}.js`);
    if (fs.existsSync(tsFile)) {
      components.push({ name: entry.name, jsPath: tsFile });
...
function main() {
  ...
  // Note: component-categories.ts is hand-maintained (was previously generated
  // as .js here, but the .ts source of truth lives at
  // components/utilities/component-categories.ts). Update it manually when
...
function writeDocsNav(outPath, categoryMap) {
  ...
  lines.push("  Loaded by <sherpa-nav data-src-html=\"/docs/nav.html\"> via fetch().");
  lines.push("");
